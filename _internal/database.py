...
def add_stock(user_id, stock_name, quantity, quantity_unit, shelf_life, date_added):
    try:
        ...
        # Calculate the expiration date based on the shelf life if provided
...
def update_stock(stock_id, name, quantity, unit, shelf_life, date_added):
    try:
        ...
        # Check if shelf_life is None to decide whether to set expiration_date
        if shelf_life is not None:
            # Calculate the expiration date only if shelf_life is provided
            expiration_date = (datetime.strptime(date_added, '%Y-%m-%d') + timedelta(days=shelf_life)).strftime('%Y-%m-%d')
        else:
            # Set expiration_date as NULL if shelf_life is not provided
...
# Function to fetch all stocks from the database
def get_all_stocks():
    try:
        conn = sqlite3.connect('coffee_shop.db')
        cursor = conn.cursor()
