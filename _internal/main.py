...
"font: 600 16pt \"Comic Sans MS\";")
        ...
        # Coffee Stirrer Line Background
        ...
        # Coffee Stirrer Available Label
...
"color: rgb(0, 0, 61);")
    ...
    def add_stock_data(self):
        try:
            ...
            # Allow `shelf_life` to be optional (e.g., empty input)
            ...
            date_added = datetime.now().strftime('%Y-%m-%d')  # Get the current date

            # Pass date_added to the add_stock function
    ...
    def update_stock_data(self):
        try:
            ...
            # Allow for a blank input to be treated as NULL
            shelf_life_text = self.update_shelf_life.text()
            shelf_life = int(shelf_life_text) if shelf_life_text else None  # Convert to int if provided, else None

            date_added = self.update_date_added.date().toString('yyyy-MM-dd')  # Retrieve the selected date

            # Use the stored stock_id to update the complete record
    ...
    def delete_stock_data(self):
        try:
            ...
            if selected_items:
                # Collect unique stock IDs from the selected rows
                stock_ids = set()
                for item in selected_items:
                    row = item.row()
                    stock_id = self.tableWidget.item(row, 0).text()  # Column 0: Stock ID
                    if stock_id:
                ...
                if stock_ids:
                    ...
                    reply = QMessageBox.question(
                        self.centralwidget,  # Use self.centralwidget or self.window() if necessary
                        'Confirm Deletion',
    ...
    def search_stock_data(self):
        try:
            ...
            search_term = self.search_field.text().strip().lower()  # Convert to lowercase for case-insensitivity

            # Get all stock data
            stocks = get_all_stocks()

            # Filter stocks based on the search term (case-insensitive)
            matching_stocks = [stock for stock in stocks if
                               search_term in str(stock[1]).lower()]  # Assuming name is at index 1

            # Clear the table before displaying search results
            self.tableWidget.setRowCount(0)

            # Populate the table with matching stocks
    ...
    def sort_table_data(self):
        try:
            ...
            # Sort the stocks based on the selected option
            if sort_option == "Default":
                # Load stocks as they are, no sorting
                stocks.sort(key=lambda x: x[0])  # Assuming the first column is the stock ID
            elif sort_option == "Date Added (Newest)":
                stocks.sort(key=lambda x: x[4], reverse=True)  # Assuming the date added is at index 4
            elif sort_option == "Date Added (Oldest)":
                stocks.sort(key=lambda x: x[4])  # Sort in ascending order
            elif sort_option == "Expiration Date (Soonest)":
                stocks.sort(key=lambda x: x[6] if x[
                                                      6] is not None else "")  # Assuming expiration date is at index 6, handle None
            elif sort_option == "Expiration Date (Latest)":
                stocks.sort(key=lambda x: x[6] if x[6] is not None else "", reverse=True)

            # Clear and repopulate the table with the sorted data
            self.tableWidget.setRowCount(len(stocks))
            headers = ["Stock\nID", "Stock\nName", "Quantity", "Unit", "Date\nAdded", "Shelf\nLife (days)",
                       "Expiration\nDate"]
            self.tableWidget.setHorizontalHeaderLabels(headers)
    ...
    def calculate_total_quantity(self, label, item_name):
        try:
            ...
            # Filter stocks by item name (case-insensitive comparison)
            total_quantity = 0
            unit = ""
            for stock in stocks:
                if stock[1].lower() == item_name.lower():  # Compare names case-insensitively
                    total_quantity += stock[2]
                    unit = stock[3]  # Assume the unit is consistent for the same item

            # Update the label with the total quantity and unit
            if total_quantity > 0:
    ...
    def handle_row_selection(self):
        try:
            ...
            if selected_items:
                ...
                # Extract data from the first item in the selected row
                selected_row = selected_items[0].row()
                stock_id = self.tableWidget.item(selected_row, 0).text()  # Column 0: Stock ID
                stock_name = self.tableWidget.item(selected_row, 1).text()  # Column 1: Stock Name
                quantity = self.tableWidget.item(selected_row, 2).text()  # Column 2: Quantity
                unit = self.tableWidget.item(selected_row, 3).text()  # Column 3: Unit
                date_added = self.tableWidget.item(selected_row, 4).text()  # Column 4: Date Added
                shelf_life = self.tableWidget.item(selected_row, 5).text()  # Column 5: Shelf Life

                # Prefill fields in the UI
                self.update_stock_name.setText(stock_name)
                self.update_quantity.setText(quantity)
                self.update_unit.setCurrentText(unit)  # Assuming this is a QComboBox
                self.update_shelf_life.setText(shelf_life)
                self.update_date_added.setDate(QDate.fromString(date_added, 'yyyy-MM-dd'))  # Set date in DateEdit

                # Store stock_id for use in the update method
    ...
    def open_login_window(self):
        from login import Ui_LoginWindow # Dynamically import to avoid circular import
